...
def get_secret(key: str) -> str | None:
    """Prefer the key pasted into the running app this session; fall back to
    an optional local secrets.toml."""
...
with st.expander("🔑 API Keys", expanded=not keys_set):
    st.text_input(
        "VirusTotal API Key",
        type="password",
        key="VIRUSTOTAL_API_KEY",
        help="Free key at virustotal.com/gui/my-apikey",
    )
    st.text_input(
        "Gemini API Key",
        type="password",
    ...
    st.caption(
        "Keys stay in this browser session only — never written to disk, logged, "
        "or sent anywhere except VirusTotal's and Gemini's own APIs."
    )
...
if analyze_clicked:
    ...
    if not is_valid:
        ...
        if not gemini_key:
            st.warning("⚠️ Paste a Gemini API key above to get an AI-generated verdict — showing raw source data only.")
        else:
